...
template <typename BasisL, typename BasisR, typename BasisS, typename BasisC, typename SiteType, typename Para>
AutoMPO get_ampo_two_Kitaev_chains (const BasisL& leadL, const BasisR& leadR, const BasisS& scatterer, const BasisC& charge, const SiteType& sites, const Para& para, const ToGlobDict& to_glob)
{
    mycheck (length(sites) == to_glob.size(), "size not match");
    ...
    {
        add_CdagC (ampo, basis1, basis2, i1, i2, coef, to_glob);
        add_CdagC (ampo, basis2, basis1, i2, i1, coef, to_glob);
    };
    add_hopping (leadL, scatterer, -1, 2, -para.tcL_up);
    add_hopping (leadL, scatterer, -2, 1, -para.tcL_dn);
    add_hopping (leadR, scatterer, 2, -1, -para.tcR_up);
    add_hopping (leadR, scatterer, 1, -2, -para.tcR_dn);
