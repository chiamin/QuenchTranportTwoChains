...
template <typename Basis1, typename Basis2, typename SiteType>
MPO get_current_mpo2 (const SiteType& sites, const Basis1& basis1, const Basis2& basis2, int i1, int i2, const ToGlobDict& to_glob)
{
    complex<Real> imag (0., 1.);
    AutoMPO ampo (sites);
    add_CdagC (ampo, basis1, basis2, i1, i2, imag, to_glob);
    add_CdagC (ampo, basis2, basis1, i2, i1, -imag, to_glob);
    auto mpo = toMPO (ampo);
    return mpo;
}

template <typename Basis1, typename Basis2, typename Basis3, typename Basis4, typename SiteType>
MPO get_current_current_mpo (const SiteType& sites,
                             const Basis1& basis1, const Basis2& basis2, int i1, int i2,
                             const Basis3& basis3, const Basis4& basis4, int i3, int i4,
                             const ToGlobDict& to_glob)
{
    ...
    add_CdagC (ampo, basis1, basis2, i1, i2, imag, to_glob);
    add_CdagC (ampo, basis2, basis1, i2, i1, -imag, to_glob);
    add_CdagC (ampo, basis3, basis4, i3, i4, imag, to_glob);
    add_CdagC (ampo, basis4, basis3, i4, i3, -imag, to_glob);
    auto mpo = toMPO (ampo);
    return mpo;
}
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        iut::read_all (ifs, psi, H, args_basis, step, to_glob, to_loc, leadL_up, leadL_dn, leadR_up, leadR_dn, scatt_up, scatt_dn, charge, scatter_sites);
    ...
    // Current MPO
    auto jmpoL_up = get_current_mpo2 (sites, leadL_up, leadL_up, -2, -1, to_glob);
    auto jmpoL_dn = get_current_mpo2 (sites, leadL_dn, leadL_dn, -2, -1, to_glob);
    auto jmpoR_up = get_current_mpo2 (sites, leadR_up, leadR_up, 1, 2, to_glob);
    auto jmpoR_dn = get_current_mpo2 (sites, leadR_dn, leadR_dn, 1, 2, to_glob);
    cout << "Current MPO bond dimension = " << maxLinkDim(jmpoL_up) << " " << maxLinkDim(jmpoL_dn) << " " << maxLinkDim(jmpoR_up) << " " << maxLinkDim(jmpoR_dn) << endl;
    ...
    while (step <= time_steps)
    {
        ...
        auto jL_up = real (innerC (psi, jmpoL_up, psi));
        auto jL_dn = real (innerC (psi, jmpoL_dn, psi));
        auto jR_up = real (innerC (psi, jmpoR_up, psi));
        auto jR_dn = real (innerC (psi, jmpoR_dn, psi));
        auto jj_upup = real (innerC (psi, prime(jmpoL_up), jmpoR_up, psi));
        auto jj_updn = real (innerC (psi, prime(jmpoL_up), jmpoR_dn, psi));
        ...
        if (write)
        {
            ...
            iut::write_all (ofs, psi, H, args_basis, step, to_glob, to_loc, leadL_up, leadL_dn, leadR_up, leadR_dn, scatt_up, scatt_dn, charge, scatter_sites);
            timer["write"].stop();
        }
    }
    timer.print();
